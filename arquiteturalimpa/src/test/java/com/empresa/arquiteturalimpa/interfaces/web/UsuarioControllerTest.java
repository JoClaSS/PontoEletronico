...
/**
 * Testes de fatia web (camada Interfaces) com o filtro JWT excluído da fatia.
 */
...
class UsuarioControllerTest {
    ...
    void deveRetornar404QuandoUsuarioNaoEncontrado() throws Exception {
        ...
        when(usuarioService.buscarPorId(id)).thenThrow(new EntityNotFoundException("Usuário não encontrado"));
    ...
    void deveRetornarUsuarioQuandoEncontrado() throws Exception {
        UUID id = UUID.randomUUID();
        UsuarioResponse usuario = UsuarioResponse.builder().id(id).nome("Ana").email("ana@empresa.com").build();
